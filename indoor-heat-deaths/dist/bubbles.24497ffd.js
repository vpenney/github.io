...
var svg = d3.select('#bubbles').append('svg').attr('width', width + margin.left + margin.right).attr('height', height + margin.top + margin.bottom).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');

var radiusScale = d3.scaleSqrt().domain([80, 125]).range([10, 20]);
...
var forceCollide = d3.forceCollide(function (d) {
  return radiusScale(d.Temp_cleaned) + 2;
});
...
// along the axes
.force('x', forceXSeparate).force('y', d3.forceY(height / 2).strength(0.03)).force('collide', forceCollide);
...
function ready(datapoints) {
  ...
  var circles = svg.selectAll('circle').data(datapoints).enter().append('circle').attr('class', function (d) {
    return d.Name;
  }).attr('fill', 'lightgrey').attr('r', function (d) {
    return radiusScale(d.Temp_cleaned);
  })

  // Start of new stuff
  .on('mouseover', function (d) {
    // Make the circle black
    d3.select(this).transition().duration(200).attr('fill', '#F2B749').attr('r', function (d) {
      return radiusScale(d.Temp_cleaned) * 1.2;
    });
    ...
    d3.select(this).transition().duration(200).attr('fill', 'lightgrey').attr('r', function (d) {
      return radiusScale(d.Temp_cleaned);
    });
  ...
  d3.select("#combined").on('click', function (d) {
    return simulation.force("x", ForceXCombine).alphaTarget(0.05).restart();
  }
  // console.log("Combine the bubbles.")
  );

  d3.select("#yearly").on('click', function (d) {
    return simulation.force("x", forceXSeparate).alphaTarget(0.25).restart();
  });
...
},{"d3":"../node_modules/d3/index.js","./maricopa_heat_deaths.csv":"maricopa_heat_deaths.csv"}],"../../../.npm-global/lib/node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../../../.npm-global/lib/node_modules/parcel/src/builtins/hmr-runtime.js","bubbles.js"], null)
//# sourceMappingURL=/bubbles.24497ffd.map